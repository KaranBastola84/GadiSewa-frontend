  ...
  return (
    <CustomerLayout pageTitle="My Profile">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-r from-sky-600 to-indigo-700 rounded-2xl p-6 sm:p-8 mb-6 text-white relative overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row items-center gap-5">
            ...
            <div className="text-center sm:text-left">
              <h2 className="text-2xl font-bold">{profile?.fullName || "Customer"}</h2>
              <p className="text-sky-100 mt-1">{profile?.email || ""}</p>
              <div className="flex flex-wrap items-center gap-2 mt-2 justify-center sm:justify-start">
                <span className="px-3 py-0.5 bg-white/15 rounded-full text-xs font-medium">Customer</span>
                <span className="px-3 py-0.5 bg-emerald-400/20 rounded-full text-xs font-medium text-emerald-200">Active</span>
                {loyaltyActive && (
                  <span className="px-3 py-0.5 bg-amber-400/20 rounded-full text-xs font-medium text-amber-200">⭐ Loyalty</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              ...
              <form onSubmit={handleSave}>
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Email Address" name="email" value={form.email} onChange={handleChange} editing={editing} type="email" />
                  <Field label="Phone Number" name="phone" value={form.phone} onChange={handleChange} editing={editing} />
                  <Field label="Address" name="address" value={form.address} onChange={handleChange} editing={editing} />
                ...
                {editing && (
                  <div className="flex gap-3 mt-5 pt-4 border-t border-slate-100">
                    ...
                    <button type="button" onClick={handleCancel} className="px-5 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-200 transition">
                      Cancel
                    </button>
                  </div>
                )}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
              <p className="text-3xl font-bold text-slate-800">{loyaltyActive ? "10%" : "0%"}</p>
              <p className="text-sm text-slate-400 mt-1">Loyalty Discount</p>
              {!loyaltyActive && (
                <p className="text-xs text-slate-400 mt-2">
                  Spend Rs. {(5000 - totalSpent).toLocaleString()} more to unlock 10% loyalty discount
                </p>
              )}
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-5 text-center">
  ...
  return (
    <div>
      ...
      {editing ? (
        ...
        <p className="px-3 py-2.5 bg-slate-50 rounded-lg text-slate-700 text-sm">{value || "—"}</p>
      )}
    </div>
  );
}
